...
class RBFFeatureEncoder:
    def __init__(self, env): # modify
        ...
        self.featurizer = sklearn.pipeline.FeatureUnion([
            ("rbf1", RBFSampler(gamma=5.0, n_components=200, random_state=1)),
            ("rbf2", RBFSampler(gamma=2.0, n_components=200, random_state=1)),
            ("rbf3", RBFSampler(gamma=1.0, n_components=300, random_state=1))
        ])
    ...
    def size(self): # modify
        ...
        return self.featurizer.transform(self.scaler.transform([self.env.observation_space.sample()])).shape[1]
...
class TDLambda_LVFA:
    ...
    def update_transition(self, s, action, s_prime, reward, done): # modify
        ...
        Q_s = self.Q(s_feats)[action]
        Q_s_prime = self.Q(s_prime_feats).max() if not done else 0
        delta = reward + self.gamma * Q_s_prime - Q_s
        # Update traces
