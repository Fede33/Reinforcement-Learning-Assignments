...
def policy_iteration(env, env_size, end_state, directions, obstacles, gamma=0.99, max_iters=1000, theta=1e-3):
    ...
    for k in range(env_size):
        for i in range(env_size):
            STATES[enumerate_states] = np.array([k, i], dtype=np.uint8)
            enumerate_states = enumerate_states + 1

    stable = False
    iter = 0

    while max_iters > iter and not stable:
        iter = iter + 1
        ...
        while True:
            ...
            for i in range(env.observation_space.n):
                ...
                p = policy[i]
                next_state_prob = transition_probabilities(env, state, p, env_size, directions, obstacles).flatten()
                values[i] = (next_state_prob * (old_val * gamma + REWARDS)).sum()

                delta = max(delta, abs(old_val[i] - values[i]))
        ...
        for i in range(env.observation_space.n):
            ...
            for k in range(env.action_space.n):
                next_state_prob = transition_probabilities(env, state, k, env_size, directions, obstacles).flatten()
                va = (next_state_prob * (gamma * values + REWARDS)).sum()
                if va > best_value:
                    best_value = va
                    best_action = k
